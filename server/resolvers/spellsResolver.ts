 ...
 */
const RANGE_CATEGORY_VALUES: Record<string, string[]> = {
 ...
 */
const DURATION_CATEGORY_VALUES: Record<string, string[]> = {
 ...
 */
const CASTING_TIME_CATEGORY_VALUES: Record<string, string[]> = {
 ...
 */
function buildRangeOr(categories: string[]): WhereClause[] {
 ...
 */
function buildDurationOr(categories: string[]): WhereClause[] {
    const conditions: WhereClause[] = [];
    for (const cat of categories) {
 ...
 */
function buildCastingTimeOr(categories: string[]): WhereClause[] {
    const conditions: WhereClause[] = [];
    for (const cat of categories) {
        if (cat === '1_reaction') {
 ...
 * active category.
 */
function buildWhere(filter: QuerySpellsArgs['filter']): WhereClause {
